...
const car = new Vehicle("BMW", "X6", 2022, 80000);
...
const myCar = new Car("BMW", "X5", 2021, 200000, "Бензин", 200);
 ...
 * | speed        |  0                  |
 */
const cars = new Car("BMW", "X7", 2022, 30000, "Mool", 0);
...
console.log(myTruck.brand);
myTruck.specific(6000);
...
Truck.prototype.tow = function (weight) {
  if (weight <= this.towingCapacity) {
    console.log("Тяга можлива. Вага тяги: " + weight);
  } else {
    console.log("Тяга не можлива. Вага тяги перевищує максимальну допустиму.");
  }
};
// Викликаємо метод tow з вагою меншою за towingCapacity
let myTrucks = new Truck(
  "Toyota",
  "Tundra",
  2019,
  20000,
...
Truck.prototype.drive = function (kilometers) {
  this.mileage += kilometers;
  console.log(
    `Подорожуємо ${kilometers} кілометрів у ${this.brand} ${this.model}.`
  );
};
let driveFunction = myTruck.drive.bind(myTruck);
...
let myElectricCar = new ElectricCar("Tesla", "Model S", 2022, 5000, 100);
 ...
 * | batteryCapacity | 100               |
 */
function ElectricCars(brand, model, year, mileage, batteryCapacity) {
  this.brand = brand;
  this.model = model;
  this.year = year;
  this.mileage = mileage;
...
ElectricCars.prototype.toString = function () {
  return `${this.brand} ${this.model} ${this.year} - Батарея: ${this.batteryCapacity} kWh`;
};
